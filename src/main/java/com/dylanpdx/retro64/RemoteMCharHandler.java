...
public class RemoteMCharHandler {
    ...
    public static Hashtable<Player, MChar> mChars = new Hashtable<>();
    ...
    public static void mCharOff(Player player, boolean fatal) {
        ...
        if (player== Minecraft.getInstance().player) {
            ...
            var smchar = mChars.remove(player);
            if(smchar != null) {
                smchar.destroy();
            }
        }
    ...
    public static void toggleMChar(Player player) {
        ...
        if (isMChar) {
            ...
            ClientPlayNetworking.send(SM64PacketHandler.TOGGLE_MARIO, buffer);
        } else  {
            mCharOn(player);
            FriendlyByteBuf buffer = new FriendlyByteBuf(Unpooled.buffer());
    ...
    public static void onMCharToggle(Player player, boolean isMChar) {
        ...
        if(ScaleRegistries.SCALE_TYPES.containsKey(heightId)) {
            ScaleData height = ScaleRegistries.SCALE_TYPES.get(heightId).getScaleData(player);
            if (isMChar) {
        ...
        FriendlyByteBuf buffer = new FriendlyByteBuf(Unpooled.buffer());
        buffer.writeBoolean(isMChar);
        if(Minecraft.getInstance().level != null) {
            ClientPlayNetworking.send(SM64PacketHandler.CHANGE_HEIGHT, buffer);
        }
        player.refreshDimensions();
    }
